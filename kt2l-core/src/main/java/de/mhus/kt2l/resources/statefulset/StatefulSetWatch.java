...
@Slf4j
public class StatefulSetWatch extends ClusterBackgroundJob {

    @Autowired
    K8sService k8s;

    @Getter
    private MEventHandler<Watch.Response<V1StatefulSet>> eventHandler = new MEventHandler<>();
        ...
        while (true) {
            try {
                ...
                for (Watch.Response<V1StatefulSet> event : watch) {
                    V1StatefulSet res = event.object;
