...
public class ShowPodsOfServiceAction implements ResourceAction {
    ...
    public void execute(ExecutionContext context) {
        ...
        final var parentSelector = ((V1Service)parent).getSpec().getSelector();
        final var uid = parent.getMetadata().getUid();
        ((ResourcesGridPanel)context.getSelectedTab().getPanel()).showResources(K8s.POD, parentNamespace, new ResourcesFilter() {
            @Override
            public boolean filter(KubernetesObject res) {
                if (res instanceof io.kubernetes.client.openapi.models.V1Pod pod) {
                    if (pod.getMetadata().getNamespace().equals(parentNamespace) &&
                            K8sUtil.matchLabels(parentSelector, pod.getMetadata().getLabels()))
                        return true;
                }
                return false;
            }
    ...
    @Override
    public String getDescription() {
        return "Show Pods of the selected Service";
    }
}
