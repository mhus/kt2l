...
@Component
@WithRole(ROLE.READ)
public class ShowJobsOfCronJobAction implements ResourceAction {
    @Override
    public boolean canHandleResourceType(Cluster cluster, K8s resourceType) {
        return K8s.CRON_JOB.equals(resourceType);
    }
    ...
    public void execute(ExecutionContext context) {
        ...
        ((ResourcesGridPanel)context.getSelectedTab().getPanel()).showResources(K8s.JOB, parent.getMetadata().getNamespace(), new ResourcesFilter() {
            @Override
            public boolean filter(KubernetesObject res) {
                if (res instanceof io.kubernetes.client.openapi.models.V1Job job) {
    ...
    public String getTitle() {
        return "Jobs;icon=" + VaadinIcon.OPEN_BOOK;
    }
    ...
    @Override
    public String getDescription() {
        return "Show Jobs of the selected Cron Job";
    }
}
