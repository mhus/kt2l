...
@Slf4j
public class JobWatch extends ClusterBackgroundJob {

    @Autowired
    K8sService k8s;

    @Getter
    private MEventHandler<Watch.Response<V1Job>> eventHandler = new MEventHandler<>();
    private Thread watchThread;
    private String clusterId;

    public static JobWatch instance(Core core, Cluster cluster) {
        return core.getBackgroundJob(cluster.getName(), JobWatch.class, () -> new JobWatch());
    }
    ...
    private void watch() {
        ...
        while (true) {
            try {
                ...
                var call = api.listJobForAllNamespaces().watch(true).buildCall(new CallBackAdapter<V1Job>(LOGGER));
