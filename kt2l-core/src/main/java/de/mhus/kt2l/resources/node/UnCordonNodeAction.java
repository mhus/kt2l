...
public class UnCordonNodeAction implements ResourceAction {
    @Override
    ...
    public void execute(ExecutionContext context) {
        ...
        var text = new Div("Do you really want to uncordon the selected " + context.getSelected().size() + " node(s)?");
        ...
        dialog.setWidth("80%");
        dialog.setConfirmText("Uncordon");
    ...
    private void drainNodes(ExecutionContext context) {
        ...
        progress.setHeaderTitle("Uncordon Nodes");
        ...
        Thread.startVirtualThread(() -> {
            for (KubernetesObject obj : context.getSelected()) {
                ...
                var uncordon = Kubectl.uncordon();
                try {
                    uncordon.apiClient(context.getCluster().getApiProvider().getClient())
                            .name(obj.getMetadata().getName()).execute();
                    context.getUi().access(() -> UiUtil.showSuccessNotification("Node " + obj.getMetadata().getName() + " uncordoned"));
                } catch (Exception e) {
                    context.getUi().access(() -> UiUtil.showErrorNotification("Error uncordon node " + obj.getMetadata().getName(), e));
                }
            }
