...
@Slf4j
public class PersistentVolumeClaimWatch extends ClusterBackgroundJob {

    @Autowired
    K8sService k8s;

    @Getter
    private MEventHandler<Watch.Response<V1PersistentVolumeClaim>> eventHandler = new MEventHandler<>();
    private Thread watchThread;
    private String clusterId;

    public static PersistentVolumeClaimWatch instance(Core core, Cluster cluster) {
        return core.getBackgroundJob(cluster.getName(), PersistentVolumeClaimWatch.class, () -> new PersistentVolumeClaimWatch());
    }
    ...
    private void watch() {
        ...
        while (true) {
            try {
                ...
                var call = api.listPersistentVolumeClaimForAllNamespaces().watch(true).buildCall(new CallBackAdapter<V1PersistentVolumeClaim>(LOGGER));
                ...
                for (Watch.Response<V1PersistentVolumeClaim> event : watch) {
                    V1PersistentVolumeClaim res = event.object;
