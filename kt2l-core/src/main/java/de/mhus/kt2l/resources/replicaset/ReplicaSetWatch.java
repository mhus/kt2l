...
@Slf4j
public class ReplicaSetWatch extends ClusterBackgroundJob {

    @Autowired
    K8sService k8s;

    @Getter
    private MEventHandler<Watch.Response<V1ReplicaSet>> eventHandler = new MEventHandler<>();
    ...
    private void watch() {
        ...
        while (true) {
            try {
                ...
                var call = api.listReplicaSetForAllNamespaces().watch(true).buildCall(new CallBackAdapter<V1ReplicaSet>(LOGGER));
                Watch<V1ReplicaSet> watch = Watch.createWatch(
                        apiProvider.getClient(),
                        call,
                        new TypeToken<Watch.Response<V1ReplicaSet>>() {
                        }.getType());

                for (Watch.Response<V1ReplicaSet> event : watch) {
                    V1ReplicaSet res = event.object;
