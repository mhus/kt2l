...
public class PersistentVolumeClaimGridFactory implements ResourceGridFactory {
    @Override
    public boolean canHandleResourceType(K8s.RESOURCE resourceType) {
        return K8s.RESOURCE.PERSISTENT_VOLUME_CLAIM.equals(resourceType);
    }

    @Override
    public ResourcesGrid create(K8s.RESOURCE resourcesType) {
        return new PersistentVolumeClaimGrid();
    }
}
