...
@Slf4j
public class ClusterRoleBindingWatch extends ClusterBackgroundJob {

    @Autowired
    K8sService k8s;

    @Getter
    private MEventHandler<Watch.Response<V1ClusterRoleBinding>> eventHandler = new MEventHandler<>();
    private Thread watchThread;
    private ApiClient client;
    private CoreV1Api api;

    public static ClusterRoleBindingWatch instance(Core core, Cluster clusterConfig) {
        return core.getBackgroundJob(clusterConfig.name(), ClusterRoleBindingWatch.class, () -> new ClusterRoleBindingWatch());
    }
    ...
    private void watch() {
        ...
        while (true) {
            try {
                ...
                for (Watch.Response<V1ClusterRoleBinding> event : watch) {
                    V1ClusterRoleBinding res = event.object;
