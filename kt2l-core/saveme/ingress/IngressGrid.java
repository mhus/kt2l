...
public class IngressGrid extends AbstractGrid<IngressGrid.Resource, Component> {
                            ...
                            MLang.tryThis(() -> createRawResourceList() )
                                    ...
                                    .onSuccess(list -> {
                                        list.getItems().forEach(res -> {
                                            IngressGrid.this.resourcesList.add(new Resource(res));
                                        });
                                    });
                        }
                        filterList();
    ...
    private V1IngressList createRawResourceList() throws ApiException {
        if (namespace == null || namespace.equals(K8s.NAMESPACE_ALL))
            return networkingV1Api.listIngressForAllNamespaces().execute();
        return networkingV1Api.listNamespacedIngress(namespace).execute();
    }
    ...
    public static class Resource {
        ...
        public Resource(V1Ingress resource) {
