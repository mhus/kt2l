...
@Slf4j
public class PersistentVolumeClaimGrid extends AbstractGrid<PersistentVolumeClaimGrid.Resource, Component> {

    private IRegistration eventRegistration;

    @Override
    protected void init() {
        eventRegistration = PersistentVolumeClaimWatch.instance(panel.getCore(), panel.getCluster()).getEventHandler().registerWeak(this::changeEvent);
    }

    private void changeEvent(Watch.Response<V1PersistentVolumeClaim> event) {
                            ...
                            MLang.tryThis(() -> createRawResourceList() )
                                    ...
                                    .onSuccess(list -> {
                                        list.getItems().forEach(res -> {
                                            PersistentVolumeClaimGrid.this.resourcesList.add(new Resource(res));
                                        });
                                    });
                        }
                        filterList();
    ...
    private V1PersistentVolumeClaimList createRawResourceList() throws ApiException {
        if (namespace == null || namespace.equals(K8s.NAMESPACE_ALL))
            return cluster.getApiProvider().getCoreV1Api().listPersistentVolumeClaimForAllNamespaces().execute();
        return cluster.getApiProvider().getCoreV1Api().listNamespacedPersistentVolumeClaim(namespace).execute();
    }
    ...
    public static class Resource {
        ...
        public Resource(V1PersistentVolumeClaim resource) {
        ...
        public boolean equals(Object o) {
            ...
            if (o instanceof V1PersistentVolumeClaim res)
